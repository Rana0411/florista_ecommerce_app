...
import '../../domain/usecase/notification_use_case.dart';
...
class NotificationsCubit extends Cubit<NotificationsState> {
  ...
  Future<void> loadNotifications() async {
    log('[NotificationsCubit] loadNotifications called, isClosed=$isClosed', name: 'Notif');
    if (isClosed) return;
    emit(state.copyWith(isLoading: true, clearError: true));
    try {
      final notifications = await _getNotifications();
      log('[NotificationsCubit] loaded ${notifications.length} notifications', name: 'Notif');
      if (isClosed) return;
      ...
      log('[NotificationsCubit] emitted state with ${notifications.length} items', name: 'Notif');
    } catch (e, st) {
      log('[NotificationsCubit] ERROR: $e', name: 'Notif', error: e, stackTrace: st);
      if (!isClosed) {
